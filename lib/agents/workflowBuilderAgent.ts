import type { DocumentSection, WorkflowStep } from "@/lib/types";
...
interface StepTemplate {
  ...
  /** preferred citation prefix for the evidence badge */
  prefer: string;
}

const WORKFLOW_TEMPLATES: Record<PersonaKey, StepTemplate[]> = {
  support: [
...
/** Pick a citation id from available sections, preferring a given prefix. */
function pickEvidence(sections: DocumentSection[], prefer: string, fallbackIndex: number): string {
  const match = sections.find((s) => s.citationId.startsWith(prefer));
  if (match) return match.citationId;
  if (sections.length > 0) return sections[fallbackIndex % sections.length].citationId;
  return `${prefer}-1`;
}

/**
 * Workflow Builder Agent.
 * Produces 4–6 persona- and market-aware workflow steps, each bound to an
 * evidence citation drawn from the retrieved sections.
 */
export function workflowBuilderAgent(
  personaKey: PersonaKey,
  _market: string,
  sections: DocumentSection[],
): WorkflowStep[] {
  const templates = WORKFLOW_TEMPLATES[personaKey] ?? WORKFLOW_TEMPLATES.support;
  // Use 4–6 steps; scale slightly with corpus breadth.
  const count = Math.min(6, Math.max(4, sections.length >= 12 ? 5 : 4 + (templates.length > 4 ? 1 : 0)));
  return templates.slice(0, count).map((t, i) => ({
    ...
    evidenceCode: pickEvidence(sections, t.prefer, i),
  }));
}
